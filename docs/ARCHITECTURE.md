...
| `/w/[ws]/b/[bundleId]` | 번들 랜딩 | 필요 |
| `/w/[ws]/b/[bundleId]/f?path=` | 번들 파일 뷰어 | 필요 |

### Bundle ID + 파일 경로 규칙

Bundle ID는 URL path의 단일 세그먼트로 사용합니다. `/`를 포함하지 않는 flat slug입니다.
파일 경로는 path segment가 아니라 query string `path`로 전달합니다.

```
/w/ci/b/pr-42-run-1                     → bundleId: pr-42-run-1, filePath: null
/w/ci/b/pr-42-run-1/f?path=a.md         → bundleId: pr-42-run-1, filePath: a.md
/w/ci/b/pr-42-run-1/f?path=logs/out.log → filePath: logs/out.log
```

**bundleId 규칙**
- 권장 정규식: `^[a-z0-9][a-z0-9._-]{0,127}$`
- 금지: `/`, `\\`, 공백, `..`, `\0`, 퍼센트 인코딩 입력값, 대문자
...
| `GET` | `/api/w/[ws]/bundles/[bundleId]/meta` | manifest + 파일 트리 | 인증 |
| `GET` | `/api/w/[ws]/bundles/[bundleId]/tree` | 파일 트리 | 인증 |
| `GET` | `/api/w/[ws]/bundles/[bundleId]/file?path=` | 파일 콘텐츠 | 인증 |
...
GET /w/[ws]/b/[bundleId]
  ...
  │
  └─ manifest.index 파일 읽어 MarkdownViewer
```

```text
GET /w/[ws]/b/[bundleId]/f?path=<filePath>
  │
  ├─ search param `path` 읽기
  ├─ extractBundle(storageKey)
  └─ detectFileType() → FileViewer 분기
```
...
FileViewer
  ...
  │                  내부 이미지 → /api/w/[ws]/bundles/[bundleId]/file?path=
...
| 라우팅 | `/b/[...segments]` | `/w/[ws]/b/[bundleId]` + `/w/[ws]/b/[bundleId]/f?path=` |

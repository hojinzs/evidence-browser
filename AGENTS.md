...
| `backend-engineer` | `packages/api/src/routes/**`, `packages/api/src/lib/**`, `packages/shared/src/**` for shared bundle/upload logic, plus QA upload helper files in `packages/cli/scripts/**`. Uses `/codex:rescue`. |
| `frontend-engineer` | `packages/web/src/**` for the SPA. Uses `/frontend-design` with `docs/DESIGN_GUIDE.md` as the SSOT. |

...
type ApiKeyScope = "read" | "upload" | "admin";
...
interface ApiKeyWithUser extends ApiKeyPublic {
  username: string;
}
...
export function ApiKeyManager({ initialKeys, users }: ApiKeyManagerProps) {
  ...
  async function handleCreate(e: { preventDefault(): void }) {
    ...
    setLoading(true);
    try {
      const body: Record<string, string> = { name, scope, userId };
      ...
      <Card className="overflow-hidden p-0">
        ...
        {initialKeys.length === 0 ? (
          ...
          initialKeys.map((k) => (
            ...
            >
              <span className="truncate text-[13px]">{k.username}</span>

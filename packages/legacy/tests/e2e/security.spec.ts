 ...
 *   `src/app/api/w/[ws]/bundles/[bundleId]/file/route.ts:10` emits
 ...
 *   lives at `/api/w/[ws]/bundles/[bundleId]/file`; auth is enforced at the
...
const FILE_API = `/api/w/${WS}/bundles/${BUNDLE_ID}/file`;
const META_API = `/api/w/${WS}/bundles/${BUNDLE_ID}/meta`;
...
test.describe("fixture-security sanitization and path safety", () => {
  test("TC-SEC-01 (AC-70) URL path traversal is blocked", async ({ page }) => {
    ...
    const targetUrl = `${BUNDLE_URL}/f?path=${encodeURIComponent("../../../etc/passwd")}`;

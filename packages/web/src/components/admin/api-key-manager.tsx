...
import type { ApiKeyScope, ApiKeyWithUser, UserPublic } from "@/lib/types";
...
function ScopeBadge({ scope }: { scope: ApiKeyScope }) {
  if (scope === "admin") {
    return <Badge className="border border-red-400/30 bg-red-400/10 text-red-400">admin</Badge>;
  }
  if (scope === "upload") {
    return <Badge className="border border-blue-400/30 bg-blue-400/10 text-blue-400">upload</Badge>;
  }
  return <Badge className="border border-border text-muted-foreground">read</Badge>;
}
...
export function ApiKeyManager({ initialKeys, users }: ApiKeyManagerProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <Card className="overflow-hidden p-0">
        <div className="grid grid-cols-[minmax(0,1.5fr)_140px_150px_80px_130px_130px_140px_56px] border-b border-border bg-white/3 px-4 py-3 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
          <span>Owner</span>
          <span>Name</span>
          <span>Prefix</span>
          <span>Scope</span>
        ...
        {initialKeys.length === 0 ? (
          ...
          initialKeys.map((key) => (
            <div
              key={key.id}
              className="grid grid-cols-[minmax(0,1.5fr)_140px_150px_80px_130px_130px_140px_56px] items-center border-b border-border px-4 py-3 last:border-b-0"
            >
              <span className="truncate text-[13px]">{key.username}</span>
              <span className="truncate text-[13px]">{key.name}</span>
              <code className="truncate font-mono text-[12px] text-muted-foreground">{key.key_prefix}</code>
              <ScopeBadge scope={key.scope} />
              <span className="truncate text-[13px] text-muted-foreground">{formatDate(key.expires_at)}</span>
      ...
      {!showForm ? (
        ...
        <Card className="p-4">
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select value={scope} onChange={(e) => setScope(e.target.value as ApiKeyScope)} className={selectClassName}>

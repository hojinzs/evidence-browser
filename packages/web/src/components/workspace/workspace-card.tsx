...
export function WorkspaceCard({ slug, name, description, bundleCount, updatedLabel }: WorkspaceCardProps) {
  return (
    <Link to={workspaceUrl(slug)} className="surface-card-hover flex h-16 items-center gap-3 border-b border-border px-4 last:border-b-0">
      ...
      {updatedLabel && <span className="hidden w-20 text-right text-xs text-[oklch(0.55_0_0)] md:block">{updatedLabel}</span>}

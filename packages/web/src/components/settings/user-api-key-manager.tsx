...
export function UserApiKeyManager({ initialKeys }: UserApiKeyManagerProps) {
  ...
  const [scope, setScope] = useState<"read" | "upload">("read");
  ...
  async function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.createApiKey({ name, scope, expiresAt: expiresAt || undefined });
    ...
    <div className="space-y-3">
      ...
      <Card className="overflow-hidden p-0">
        ...
        {initialKeys.length === 0 ? (
          <div className="px-4 py-8 text-center text-[13px] text-muted-foreground">No API keys yet. Create one below.</div>
        ) : (
          initialKeys.map((key) => (
            <div
              key={key.id}
        ...
        <Card className="p-4">
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select value={scope} onChange={(e) => setScope(e.target.value as "read" | "upload")} className={selectClassName}>

...
export function BundleCard({ title, bundleId, href, uploadedBy, createdAt, sizeBytes }: BundleCardProps) {
  const state = /fail|error/i.test(`${title} ${bundleId}`) ? "failed" : /pass|success/i.test(`${title} ${bundleId}`) ? "passed" : "bundle";

  return (
    <Link to={href} className="surface-card-hover flex items-center gap-4 border-b border-border px-5 py-3.5 last:border-b-0">

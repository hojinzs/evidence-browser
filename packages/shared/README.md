...
When adding a shared module, add the granular subpath to `package.json` `exports` and re-export that same source module from `src/index.ts`. The shared package build runs `scripts/check-export-surface.mjs`, which fails if a package subpath export is missing from the emitted root barrel.
